...
public class ArbritaryGraph extends Graph<int[]>
{
    ...
    {
        return BasicOrientedGraph.noValue();
    }
        ...
        for (int i = 0; i < size(); i++)
        {
            for (int j = 0; j < size(); j++)
            {
                try
                {
                    ...
                    if (getParamAt(i, j, numParamStream) < getParamAt(i, j, numParamCapacity))
                    { //arc non saturé
    ...
    {
        BasicOrientedGraph oivGraph = getOrientedIntValuedGraphBy(0);
        FloydWarshall tmpFloyd = new FloydWarshall(oivGraph);
        tmpFloyd.runAlgorithm();
        int[][] tmpDelta = tmpFloyd.getDelta();
        int tmpSource = source;

        while (tmpDelta[source][target] == BasicOrientedGraph.noValue())
        {

            int k = generateInteger(0, tmpDelta.length - 1);
            if (tmpDelta[tmpSource][k] == BasicOrientedGraph.noValue())
            {
    ...
    public ArbritaryGraph generateRandomGraph(int source, int target, int minCost, int maxCost, int minCapacity, int maxCapacity)
    {
        ...
        int edgesToAdd = generateInteger(0, this.size() * (this.size() - 1));
     ...
     * @return
     */
    public BasicOrientedGraph getOrientedIntValuedGraphBy(int numParam)
    {
        ...
        return new BasicOrientedGraph(tmpMatrix);
    }
